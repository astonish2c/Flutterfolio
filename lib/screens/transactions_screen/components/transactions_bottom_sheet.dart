...
class TransactionsBottomSheet extends StatefulWidget {
  const TransactionsBottomSheet({super.key, required this.coin, required this.indexTransaction, required this.popPage});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          TransactionsBottomSheetRow(title1: 'Type', title2: transaction.isSell ? 'Sell' : 'Buy'),
          const Divider(color: Colors.grey),
          const SizedBox(height: 8),
          TransactionsBottomSheetRow(title1: 'Date', title2: DateFormat('d, MMM, y, h:m a').format(transaction.dateTime)),
          const Divider(color: Colors.grey),
          const SizedBox(height: 8),
          TransactionsBottomSheetRow(title1: 'Price Per Coin', title2: currencyConverter(transaction.buyPrice)),
          const Divider(color: Colors.grey),
          const SizedBox(height: 8),
          TransactionsBottomSheetRow(title1: 'Quantity', title2: currencyConverter(transaction.amount, isCurrency: false)),
          const Divider(color: Colors.grey),
          const SizedBox(height: 8),
          const TransactionsBottomSheetRow(title1: 'Fee', title2: 'No Fee'),
          const Divider(color: Colors.grey),
          const SizedBox(height: 8),
          TransactionsBottomSheetRow(title1: 'Total Cost', title2: currencyConverter(transaction.amount * transaction.buyPrice)),
          const Divider(color: Colors.grey),
          const SizedBox(height: 16),
          CustomBigButton(
            ...
            onTap: isLoading
                ...
                : () {
                    ...
                    if (transaction.isSell) {
                      Navigator.of(context).pushNamed(TabScreen.routeName, arguments: {'coinModel': widget.coin, 'indexTransaction': widget.indexTransaction, 'initialPage': 1});
                    } else {
                      Navigator.of(context).pushNamed(TabScreen.routeName, arguments: {'coinModel': widget.coin, 'indexTransaction': widget.indexTransaction, 'initialPage': 0});
                    }
                  },
          ),
          const SizedBox(height: 8),
